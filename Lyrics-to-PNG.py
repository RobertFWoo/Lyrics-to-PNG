...
FONT_SIZE = 150
FONT_PATH = "/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf"  # fallback if Georgia is not available
...
def get_font():
    # Try Georgia Bold, fallback to DejaVuSerif-Bold
    try:
        return ImageFont.truetype("Georgia Bold.ttf", FONT_SIZE)
    except:
        try:
            return ImageFont.truetype("Georgia-Bold.ttf", FONT_SIZE)
        except:
            return ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf", FONT_SIZE)
...
def draw_text_with_effects(draw, text, font, position):
    ...
    shadow_draw.text(shadow_pos, text, font=font, fill=SHADOW_COLOR, stroke_width=STROKE_WIDTH, stroke_fill=STROKE_COLOR)
    shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(radius=SHADOW_BLUR))

    # Bevel (simulate by drawing lighter text slightly offset)
    bevel_layer = Image.new("RGBA", (IMG_WIDTH, IMG_HEIGHT), (0, 0, 0, 0))
    bevel_draw = ImageDraw.Draw(bevel_layer)
    bevel_offset = (position[0] - 2, position[1] - 2)
    bevel_draw.text(bevel_offset, text, font=font, fill=(255,255,255,80), stroke_width=0)

    # Main text
    text_layer = Image.new("RGBA", (IMG_WIDTH, IMG_HEIGHT), (0, 0, 0, 0))
    text_draw = ImageDraw.Draw(text_layer)
    text_draw.text(position, text, font=font, fill=TEXT_COLOR, stroke_width=STROKE_WIDTH, stroke_fill=STROKE_COLOR)
...
def main():
    ...
    # Draw title image (multi-line, centered horizontally per line)
    img = Image.new("RGBA", (IMG_WIDTH, IMG_HEIGHT), BG_COLOR)  # Use solid background
    draw = ImageDraw.Draw(img)
    line_heights = []
    for line in title_lines:
        bbox = draw.textbbox((0, 0), line, font=font, stroke_width=STROKE_WIDTH)
        line_heights.append(bbox[3] - bbox[1])
    total_height = sum(line_heights) + (len(title_lines) - 1) * 10  # 10px spacing
